(()=>{
  ...
  function resolvedTheme(mode=getTheme()){return mode==='system'?(window.matchMedia('(prefers-color-scheme: light)').matches?'light':'dark'):mode}
  ...
  async function loadContent(){try{const r=await fetch('/api/content',{cache:'no-store'});if(r.ok){const j=await r.json();content=j.items||[];applyContent(getLang())}}catch{}}
  ...
  function injectThemeStyles(){
    ...
    st.textContent=`
      ...
      html[data-theme="light"]{--z-bg:#f5f2eb;--z-surface:#fffdf8;--z-surface2:#f0ece3;--z-text:#171717;--z-muted:#68635b;--z-line:#d8d1c4}
      body,body.home-v2{background:var(--z-bg)!important;color:var(--z-text)!important;transition:background .22s,color .22s}
      ...
      html[data-theme="light"] h1,html[data-theme="light"] h2,html[data-theme="light"] h3,html[data-theme="light"] h4,html[data-theme="light"] h5,html[data-theme="light"] h6,html[data-theme="light"] .title,html[data-theme="light"] .panel h3{color:#171717!important}
      html[data-theme="light"] p,html[data-theme="light"] .lead,html[data-theme="light"] .muted,html[data-theme="light"] .panel p,html[data-theme="light"] .note{color:#68635b!important}
      html[data-theme="light"] .top,html[data-theme="light"] .home-v2 .top{background:#faf8f2f2!important;border-bottom-color:#d8d2c6!important}
      ...
      html[data-theme="light"] .brand-row{background:#0b0b0b;border:1px solid #252525;border-radius:999px;padding:7px 13px}
      html[data-theme="light"] .hero:not(.hv-hero){background:#f7f4ed!important;border-bottom-color:#d8d2c6!important;box-shadow:none!important}
      ...
      html[data-theme="light"] .card:not(.hv-card),html[data-theme="light"] .panel,html[data-theme="light"] .metric,html[data-theme="light"] .form-card,html[data-theme="light"] .ai-shell,html[data-theme="light"] .hv-owner-copy,html[data-theme="light"] .hv-location,html[data-theme="light"] .hv-contact-box{background:#fffdf8!important;border-color:#d8d1c4!important;color:#171717!important;box-shadow:0 16px 42px #5d4b2412}
      ...
      html[data-theme="light"] .hv-copy h1,html[data-theme="light"] .hv-copy h1 span{color:#fff!important}html[data-theme="light"] .hv-copy p{color:#b4b4b4!important}
      html[data-theme="light"] .hv-card{color:#fff!important;border-color:#383838!important;box-shadow:none!important}
      html[data-theme="light"] .hv-card h3{color:#fff!important}html[data-theme="light"] .hv-card p{color:#c0c0c0!important}html[data-theme="light"] .hv-card .more{color:var(--z-accent)!important}
      html[data-theme="light"] .hv-card .ico{background:#070707d8!important;border-color:var(--z-accent-deep)!important;color:var(--z-accent)!important}
      html[data-theme="light"] .hv-owner-visual{background:linear-gradient(90deg,#08080822,#080808aa),url('https://www.mountainwestdetailing.com/assets/hero-car-DDPNeaoz.jpg') center/cover no-repeat!important;box-shadow:none!important}
      html[data-theme="light"] .hv-owner-visual.has-image{background:linear-gradient(90deg,#08080822,#080808aa),var(--z-content-image) center/cover no-repeat!important}
      html[data-theme="light"] .hv-owner-visual:before{color:#ffffff08!important}
      html[data-theme="light"] .hv-location:before{filter:none!important;opacity:1!important}
  ...
  function addAppearanceDock(){
    if(document.querySelector('.z-appearance-dock'))return;
    const dock=document.createElement('div');dock.className='z-appearance-dock';
    dock.innerHTML=`
      <div class="z-quick-modes" role="group" aria-label="Appearance">
        <button data-theme-mode="system" title="Automatic / System">A</button>
        <button data-theme-mode="dark" title="Dark">●</button>
        <button data-theme-mode="light" title="Light">☀</button>
      </div>
      <div class="z-color-wrap">
        <button class="z-color-btn" aria-label="Website color" title="Website color">◉</button>
        <div class="z-color-panel">
          <span class="z-color-title" data-ku="ڕەنگی وێبسایت" data-ar="لون الموقع" data-en="Website color"></span>
          <div class="z-color-swatches">
            <button class="z-swatch" data-palette-choice="gold" aria-label="Gold"></button>
            <button class="z-swatch" data-palette-choice="blue" aria-label="Blue"></button>
            <button class="z-swatch" data-palette-choice="red" aria-label="Red"></button>
            <button class="z-swatch" data-palette-choice="green" aria-label="Green"></button>
            <button class="z-swatch" data-palette-choice="purple" aria-label="Purple"></button>
          </div>
        </div>
      </div>`;
    document.body.appendChild(dock);
    dock.querySelectorAll('[data-theme-mode]').forEach(b=>b.addEventListener('click',()=>applyTheme(b.dataset.themeMode,true)));
    const wrap=dock.querySelector('.z-color-wrap'),btn=dock.querySelector('.z-color-btn');
    btn.addEventListener('click',e=>{e.stopPropagation();wrap.classList.toggle('open')});
    dock.querySelectorAll('[data-palette-choice]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();applyColor(b.dataset.paletteChoice,true);wrap.classList.remove('open')}));
    document.addEventListener('click',e=>{if(!wrap.contains(e.target))wrap.classList.remove('open')});
    applyTheme(getTheme(),false);applyColor(getColor(),false);
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    applyLang(saved||'ku',false);applyTheme(getTheme(),false);applyColor(getColor(),false);
