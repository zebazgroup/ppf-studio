...
app.get('/admin/api/qr',auth,(req,res)=>res.json({ok:true,ready:true,database:true}));app.get('/status',async(req,res)=>{try{await pool.query('SELECT 1');res.json({ready:true,database:true})}catch{res.status(503).json({ready:false,database:false})}});
...
app.get('/admin',(req,res)=>res.sendFile(path.join(__dirname,'public','admin.html')));app.use((req,res)=>res.sendFile(path.join(__dirname,'public','index.html')));const port=process.env.PORT||3000;app.listen(port,'0.0.0.0',()=>console.log(`ZEBAZ PPF Studio running on port ${port}`));