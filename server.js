...
function requestType(b){
  if(['ppf','studio'].includes(b?.requestType))return b.requestType;
  const c=clean(b?.serviceCode||b?.service).toLowerCase();
  return /(review|advert|ڕیڤیو|ڕیکلام|مراجعة|إعلان)/i.test(c)?'studio':'ppf';
}
...
app.get('/content-image/:slot',async(req,res)=>{
  try{
    ...
    const {rows}=await pool.query(`SELECT image,mime_type FROM site_content WHERE slot=$1 AND visible=TRUE`,[slot]);
...
app.post('/api/ai',async(req,res)=>{
  ...
  const input=[];
  for(const h of history){const role=h?.role==='assistant'?'assistant':'user',txt=clean(h?.content,1800);if(txt)input.push({role,content:[{type:role==='assistant'?'output_text':'input_text',text:txt}]});}
  input.push({role:'user',content:[{type:'input_text',text:message}]});
  try{
    const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5.6-terra',instructions:ZEBAZ_AI_INSTRUCTIONS,input,tools:[{type:'web_search'}],tool_choice:'auto',max_output_tokens:1400})});
    ...
    const answer=extractResponseText(data);if(!answer)return res.status(502).json({ok:false,error:'No AI response received'});res.json({ok:true,answer});
